...
export function NewsletterSignup({ variant = "default" }: { variant?: "default" | "dark" }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const source = typeof window !== "undefined" ? window.location.pathname : "website";
  ...
  return (
    <section className={`section-padding ${isDark ? "bg-deep text-deep-foreground" : "bg-soft"}`}>
      <div className="container-calm max-w-2xl text-center">
        ...
        {status === "success" ? (
          <p className={`text-sm font-medium ${isDark ? "text-deep-foreground/80" : "text-foreground"}`}>
            Tak — du er tilmeldt Langsomt Brev.
          </p>
        ) : (
          <form onSubmit={handleSubmit} className="space-y-3 max-w-md mx-auto">
            ...
            >
              {status === "loading" ? "Tilmelder…" : "Modtag Langsomt Brev"}
            </Button>
          </form>
        )}
